		...
		PfcpFTeid *tempfTeid = (PfcpFTeid *)establishRequest->createPDR[0].pDI.localFTEID.value;
			...
			if(tempfTeid->ch == 1){
				...
				inet_pton(AF_INET,"10.144.175.51", &fTeid.addr4);
     				//status = PfcpSockaddrToFTeid(Self()->, NULL, &fTeid, &len);
			
				
				//fTeid.addr4 = Self()->pfcpAddr->s4.sin_addr;//assign ipv4
 				
	} 
	else response->createdPDR.localFTEID = establishRequest->createPDR[0].pDI.localFTEID;
